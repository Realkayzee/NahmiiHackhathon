...
contract OlympusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SNII }

    address public immutable NII;
    ...
    address public sNII;
    uint public sNIIQueue; // Delays change to sNII address
    ...
    constructor (
        ...
        require( _NII != address(0) );
        NII = _NII;

        isReserveToken[ _DAI ] = true;
        reserveTokens.push( _DAI );

        isLiquidityToken[ _NIIDAI ] = true;
        liquidityTokens.push( _NIIDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for NII
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint NII needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( NII ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn NII for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        INIIERC20( NII ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sNII ).balanceOf( msg.sender ); // Can only borrow against sNII held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with NII
        @param _amount uint
     */
    function repayDebtWithNII( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        INIIERC20( NII ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, NII, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( NII ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( NII ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns NII valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match NII decimals
            value_ = _amount.mul( 10 ** IERC20( NII ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SNII ) { // 9
            sNIIQueue = block.number.add( blocksNeededForQueue );
        } else return false;
